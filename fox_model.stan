...
P[1] ~ lognormal(0,sqrt(sigma2))T[0.5,2.0]; //prior for initial population size

// state process
for (i in 2:(N+M)) {
P[i] ~ lognormal(fmax(log(P[i-1] + r*P[i-1]*(1-log(P[i-1]*K)/log(K)) - C[i-1]/K),log(0.01)),sqrt(sigma2));
}
